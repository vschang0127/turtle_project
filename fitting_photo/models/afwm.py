    ...
    def forward(self, x, x_warps, x_conds, warp_feature=True):
        ...
        for i in range(len(x_warps)):
          ...
          if last_flow is not None and warp_feature:
              x_warp_after = F.grid_sample(x_warp, last_flow.detach().permute(0, 2, 3, 1),align_corners=True,
          ...
          if last_flow is not None:
              flow = F.grid_sample(last_flow, flow, mode='bilinear',align_corners=True, padding_mode='border')
          else:
              flow = flow.permute(0, 3, 1, 2)

          last_flow = flow
          x_warp = F.grid_sample(x_warp, flow.permute(0, 2, 3, 1),align_corners=True,mode='bilinear', padding_mode='border')
        ...
        x_warp = F.grid_sample(x, last_flow.permute(0, 2, 3, 1),align_corners=True,
                     mode='bilinear', padding_mode='border')
